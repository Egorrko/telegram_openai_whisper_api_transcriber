...
# nodejs/npm are required by phoenix_vite to build the LiveVue assets.
RUN apt-get update \
...
# assets.setup runs `npm install` at the project root, where LiveVue's
# package.json lives. It resolves live_vue/phoenix/phoenix_vite through
# `file:./deps/*`, so it has to run after deps are fetched.
# The stock Dockerfile assumes esbuild, which needs no npm at all.
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/telegram_voice_transcriber_ash ./
